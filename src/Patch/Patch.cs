...
public class Patch_SteamManager_Awake {
	...
	public static void Postfix(SteamManager __instance) {
		MPMain.Logger.LogInfo($"[Patch] SteamManager.Awake called, preparing to inject core.");

		if (_hasCoreInjected) {
			MPMain.Logger.LogWarning("[Patch] MPCore already injected, skipping.");
			return;
		}
		...
		if (existingCore != null) {
			MPMain.Logger.LogWarning($"[Patch] MPCore instance already exists. GameObjectName: {existingCore.name}");
		...
		try {
			...
			MPMain.Logger.LogInfo($"[Patch] MPCore object successfully injected into SteamManager.");
			_hasCoreInjected = true;

		} catch (System.Exception e) {
			MPMain.Logger.LogError("[Patch] Failed to inject MPCore: " + e);
		}
	}
}
